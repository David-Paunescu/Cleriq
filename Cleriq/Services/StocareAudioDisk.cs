...
public class StocareAudioDisk : IStocareAudio
{
    private readonly string _caleRoot;
    private readonly ILogger<StocareAudioDisk> _logger;

    public StocareAudioDisk(IConfiguration config, ILogger<StocareAudioDisk> logger)
    {
        _logger = logger;

        var configurat = config["DirectorAudio:CaleRoot"];
        if (string.IsNullOrWhiteSpace(configurat))
            throw new InvalidOperationException(
                "Configurare lipsă: DirectorAudio:CaleRoot în appsettings.");
    ...
    public async Task<FisierAudio> SalveazaAsync(
        int institutieId, string numeFisierOriginal, Stream continut, CancellationToken ct = default)
    {
        ...
        var cheieRelativa = Path.GetRelativePath(_caleRoot, caleAbsoluta).Replace('\\', '/');

        _logger.LogInformation(
            "Audio stocat: tenant={Tenant}, cheie={Cheie}, marime={Marime}B",
            institutieId, cheieRelativa, total);

        return new FisierAudio(cheieRelativa, total, hash);
    }
    ...
    public Task<Stream> DeschideAsync(string cheie, CancellationToken ct = default)
    {
        ...
        if (!File.Exists(caleAbsoluta))
            throw new FileNotFoundException("Fișierul audio nu există pe disk.", cheie);
