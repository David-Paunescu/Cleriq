...
export interface DateAtribuireNumarDialog {
  hclId: number;
}

@Component({
  selector: 'app-atribuie-numar-dialog',
  imports: [
...
export class AtribuieNumarDialog {
  private readonly api = inject(HclService);
  private readonly dialogRef = inject(MatDialogRef<AtribuieNumarDialog, HclDetalii>);
  private readonly date = inject<DateAtribuireNumarDialog>(MAT_DIALOG_DATA);
  ...
  private async incarcaSugestie(): Promise<void> {
    ...
    try {
      const s = await this.api.sugestieNumar(this.date.hclId);
      this.an.set(s.an);
  ...
  async atribuie(confirmaCuLacune = false): Promise<void> {
    ...
    try {
      const rezultat = await this.api.atribuieNumar(this.date.hclId, { numar, confirmaCuLacune });
