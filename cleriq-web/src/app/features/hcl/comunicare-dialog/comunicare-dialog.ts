...
export interface DateComunicareDialog {
  hclId: number;
}

@Component({
  selector: 'app-comunicare-dialog',
  imports: [
...
export class ComunicareDialog {
  private readonly api = inject(HclService);
  private readonly dialogRef = inject(MatDialogRef<ComunicareDialog, ComunicareHclPrefect>);
  private readonly date = inject<DateComunicareDialog>(MAT_DIALOG_DATA);
  ...
  async adauga(): Promise<void> {
    ...
    try {
      const rezultat = await this.api.adaugaComunicare(this.date.hclId, {
