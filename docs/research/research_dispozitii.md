...
| Erata = act nou, numerotare proprie, legat de original (Legea 24/2000 art. 71) | **DA, identic** | Faza 7, pattern unificat PV/HCL/Dispoziție |
...
- `EstePublicat` + `DataPublicareMol` au sens **doar pentru `TipDispozitie.Normativ`**. La `Individual`, încercarea de publicare în MOL/portal → **409/400** (gardă de tip).
...
**Dispoziția individuală — efecte de la comunicarea către destinatar.** *[TEXT DE LEGE]* Art. 199 alin. (1): „Comunicarea hotărârilor şi **dispoziţiilor cu caracter individual către persoanele cărora li se adresează** se face în cel mult **5 zile de la data comunicării oficiale către prefect**." Principiul: actul individual produce efecte juridice **de la data comunicării către destinatar** (paralel cu art. 199 alin. (2) pentru HCL individual). *[ZONĂ DE INCERTITUDINE — confirmare avocat]* Pentru actele de personal, comunicarea și efectele sunt guvernate suplimentar de legislația funcției publice / Codul muncii (Partea a VI-a din Cod, după caz); pot exista regimuri speciale de comunicare/contestare. Pentru design e suficient principiul: **efecte de la comunicarea către destinatar**.
...
**Concluzie pe tip de act.** *[INTERPRETARE PROPRIE + confirmare empirică]* Convocarea de către primar (ordinară + extraordinară lit. a) și c) **ESTE o dispoziție a primarului** — act administrativ **individual / de organizare** (vizează o ședință determinată, persoane determinate). În practică se emite cu temei expres art. 196 alin. (1) lit. b) și formula „DISPUNE / Se convoacă …" (confirmat: Dispoziția PMB nr. 1586/2024 „privind convocarea CGMB"). **Excepție:** convocarea inițiată de 1/3 din consilieri (art. 133 alin. (2) lit. b) **NU** e dispoziție — e „document de convocare semnat de consilieri" (art. 134 alin. (1) lit. b). Deci nu orice convocare = dispoziție.
...
- **Gardă dură de tip la publicare:** `TipDispozitie.Individual` → **niciodată** publicare în MOL/portal (409). Aceasta nu e doar regulă de business „nice-to-have", ci aliniere la Anexa 1 + Legea 190/2018.
...
**Dispoziție INDIVIDUALĂ** (act de personal — fără publicare):

| Stare | Editare conținut | Ștergere/înlocuire PDF semnat | „Anulează MOL" | Publicare MOL/portal |
|---|---|---|---|---|
| Draft / Numerotat | Permis | Permis (nu a circulat) | N/A | **INTERZIS** (gardă de tip, 409) |
| Semnat, NEcomunicat | Permis cu motiv în audit | Permis cu motiv obligatoriu | **N/A** (nu se publică) | **INTERZIS** |
| Comunicat destinatarului / în vigoare | **BLOCAT** | **BLOCAT TOTAL** | **N/A** | **INTERZIS** |
...
3. **`TipDispozitie { Normativ=1, Individual=2 }` ca discriminator de regim**, NU de numerotare. Garda dură: `Individual` ⇒ publicare MOL/portal interzisă (409). `EstePublicat`/`DataPublicareMol` au sens doar pentru `Normativ`.
...
8. **„Atacat de prefect + suspendat de drept"** = **stare/relație temporară**, NU `MotivInvalidare` (transfer identic din `research_MotivInvalidare`).
...
10. **Protecția datelor (must-have):** individualele de personal nu pleacă pe rute publice; minimizare în orice expunere (registru = doar metadate); control de acces intern; audit pe accesul la acte de personal; conformitate Legea 190/2018 art. 4/7 (DPO, termene de stocare, măsuri tehnice).
...
2. **Publicare** → Normativ în MOL (Anexa 1 lit. d + art. 197 alin. (5)); **Individual NU se publică** (text de lege, nu doar bună practică). Individualele nu ajung nici pe portal (Faza 9).
...
**ZONĂ DE INCERTITUDINE — confirmare avocat:**
- Granița exactă a momentului „a produs efecte" pentru dispoziția individuală vs. normativă (fereastra semnare → comunicare la prefect → comunicare destinatar) — recomand pragul de blocare la cel mai timpuriu moment verificabil, pentru siguranță.
- Regimul special de comunicare/contestare al actelor de personal (interferența cu legislația funcției publice / Codul muncii, Partea a VI-a din Cod) — nu schimbă principiul „efecte de la comunicare", dar poate adăuga termene/proceduri proprii.
- Tratarea `Inexistent` pentru o dispoziție individuală (care oricum nu se publică) — aplicabilitatea sancțiunii inexistenței la individuale, vs. doar la normative nepublicate, e o extindere care necesită confirmare.
- Caducitatea NU e reglementată text-expres în dreptul român (construcție doctrinară — Podaru); implementabilă și utilă, dar de semnalat ca atare.
- Codul de procedură administrativă (proiect în dezbatere la Senat la iunie 2026) — dacă va reglementa expres „rectificarea actelor administrative" și emiterea/controlul actelor, reevaluați fluxul de erată și pragurile de blocare după forma adoptată.

> **Recomandare finală:** validare juridică cu un avocat de drept administrativ pentru (a) pragul exact de înghețare per tip, (b) regulile de validare pe `reclamant` la `AnulatDeInstanta`, (c) regimul de comunicare al actelor de personal. Restul concluziilor (semnatari, publicare doar normativ, comunicare ambele la prefect, registru unic) sunt ancorate în text de lege și pot fundamenta direct designul backend.
